...
import OfferListSummary from "./ListSummary";
...
class SearchOffer extends React.Component{
    constructor(props){
        super(props);
        this.state = {
            searchResult: { offers: [], totalRecord: 0 },
            detail: {},
    ...
    load(pageNumber) {
        OfferService.search(this.state.keyword,this.state.value,pageNumber)
        .then((result) => {
            this.setState({ searchResult: result, activePage: pageNumber, project:[] })
            if(this.state.searchResult.offers && this.state.searchResult.offers.length ){
                this.getDetail(this.state.searchResult.offers[0].id);
            }else{
    ...
    render() {
        ...
        return (
            <Container fluid className="main-content-container">
                ...
                {searchResult.offers!=null && searchResult.offers.length != 0 && 
                    <Row className="py-4">                   
                        <Col md="5" style={{ padding: "0 2px" }}>
                            <OfferListSummary projects={searchResult.offers} getDetail={this.getDetail} showAlert={this.showAlert} />
                            <div style={{marginTop:"1rem"}}> <Pagination>{items}</Pagination> </div>
                        </Col>
                        <Col md="7" style={{ padding: "0 2px" }}>                       
                            <OfferDetail offer={detail} accountId={this.props.accountId} showAlert={this.showAlert} />
                        </Col>
                    </Row>
                    }
                </div>
